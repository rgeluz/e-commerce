

//Group members	
	Roman Geluz 
	ID# 21849143

	Rishi Shah
	ID# 12474679
	
	Michael Hernandez
	ID# 68363688
	
	Haitao Ye
	ID# 21497606

//-------------------------------//	
//	About the site for PA-4
//-------------------------------//
	//MySQL Database
	SQL Script is located at PA-4/Gamehub/WebContent/sql/ecommerce_05302020.sql
	Servername = 127.0.0.1
	Username = root
	Passowrd = testdb
	Databasename = ecommerce


	We took the original PA-3 project and for PA-4, we separted it into two projects "GameHub4" and "EcommerceAPI". "GameHub4" will continue to be the ecommerce website, with the jsp files and servlets. It will also serve as the REST client for the "EcommerceAPI", which is the REST API for the ecommerce website. 
	Data Persistence and Retrieval now occurs inside of the "EcommerceAPI", so all database access to the ecommerce database instance in mysql resides in "EcommerceAPI" 

	//-------------------------------//
		Requirement #1
	//-------------------------------//
		All of our product category pages for categories Accessories, Apparel, PC Gaming, PlayStation 4, Nintendo Switch and Microsoft XBox have been reimplemented as jsp files. See jsp files with "productcategory_" prefix. Each product category shows one or more product cards and when a product card is clicked, the user is taken to a dynamically generated page, which is generated by a single jsp file "productdetails.jsp"
		
	//-------------------------------//
		Requirement #2
	//-------------------------------//
		The EcommerceAPI project has about seven REST methods:

		
		OrderResource.getOrderByID(Integer orderID)
			GET method
			http://localhost:8090/EcommerceAPI/v1/api/orders/id/{orderid}
			exp:

                        http://localhost:8090/EcommerceAPI/v1/api/orders/id/70

                        {
                            "orderid": 70,
                            "todaysDate": "2020-05-23",
                            "orderproductname": "See ProductList for complete product list",
                            "orderpricequantity": 599.9,
                            "orderdiscount": 0.0,
                            "ordersubtotal_afterdiscount": 599.9,
                            "ordershippingprice": 11.0,
                            "ordersubtotal_aftershipping": 610.9,
                            "ordertaxrate": 0.07,
                            "orderamounttaxed": 44.29,
                            "ordersubtotal_aftertax": 655.19,
                            "ordertotalprice": 655.19,
                            "firstname": "a",
                            "lastname": "b",
                            "address": "5555 Main St",
                            "city": "Irvine",
                            "state": "California",
                            "zipInt": 10000,
                            "shippingmethod": "overnight",
                            "orderproductid": "See ProductList for complete produt list",
                            "productIDList": "AC8X1245,FFRE4212,",
                            "orderquantity": 0,
                            "quantityList": "5,5,",
                            "cardnumber": "1111-2222-3333-4444",
                            "expmonth": "September",
                            "expyearInt": 2020,
                            "cvvInt": 352,
                            "phone": "1-555-555-5555",
                            "email": "johndoe55@example.com"
                        }




		OrderResource.addOrder(Order order)
			POST method
			http://localhost:8090/EcommerceAPI/v1/api/orders
			exp:

                        http://localhost:8090/EcommerceAPI/v1/api/orders

                        104   //This is the record id of the newly created "order" record




		ProductResource.getProductByID(String productID)
			GET method
			http://localhost:8090/EcommerceAPI/v1/api/products/id/{productid}
			exp:

			                        http://localhost:8090/EcommerceAPI/v1/api/products/id/11VX1635

                        {
                            "productName": "Arozzi - Verona Pro V2 Gaming Chair - Green",
                            "quantity": 100,
                            "productID": "11VX1635",
                            "productCategory": "Accessories",
                            "category": "Gaming Chair",
                            "platform": "",
                            "price": 249.99,
                            "description": "Experience exceptional comfort by sitting on this Arozzi Verona Pro V2 green gaming chair.The neck and lower-back cushions provide extra support, while the dual-wheel casters ensure rolling around your setup is smooth and simple. This Arozzi Verona 				Pro V2 green gaming chair features adjustable height and tilt for optimal sitting.",
                            "imageLinks": "../img/products/arozzi/arozzi1.jpg,../img/products/arozzi/arozzi1.jpg,../img/products/arozzi/arozzi2.jpg,../img/products/arozzi/arozzi3.jpg,../img/products/arozzi/arozzi4.jpg"
                        }


		ProductResource.getProductsByCategory(String category)
			GET method
			http://localhost:8090/EcommerceAPI/v1/api/products/category/{category}
			exp:

			                        http://localhost:8090/EcommerceAPI/v1/api/products/category/Apparel

                        [
                            {
                                "productName": "Vintage Metroid cotton t-shirt",
                                "quantity": 100,
                                "productID": "56SH682",
                                "productCategory": "Apparel",
                                "category": "RPG",
                                "platform": "",
                                "price": 18.63,
                                "description": "Vintage Metroid cotton t-shirt. The design is based on the action-adventure game Metroid, developed and published by Nintendo.",
                                "imageLinks": "../img/products/metroid/metroid.png,../img/products/metroid/metroid.png,../img/products/metroid/metroid2.png"
                            },
                            {
                                "productName": "Street Fighter Group Shot Men's T-Shirt",
                                "quantity": 100,
                                "productID": "89GR1235",
                                "productCategory": "Apparel",
                                "category": "Vintage Games",
                                "platform": "",
                                "price": 21.99,
                                "description": "Now you can rock a piece of gaming history with this slick Street Fighter II men’s tee shirt. Featuring the whole gang ready to rumble, this comfy cotton black t-shirt will have you googling the nearest arcade.",
                                "imageLinks": "../img/products/streetfighter/streetfighter2.png,../img/products/streetfighter/streetfighter2.png"
                            },
                            {
                                "productName": "Minecraft Creeper T-Shirt",
                                "quantity": 100,
                                "productID": "89GX1MN5",
                                "productCategory": "Apparel",
                                "category": "Video Games",
                                "platform": "",
                                "price": 10.0,
                                "description": "Creepers are one of the most unique and iconic hostile mobs found in Minecraft.\r\nA Creeper can easily be recognized by its tall vertical structure (roughly the size of a player), green, pixelated skin, and four legs. It is possibly the most 					dangerous enemy in the game.",
                                "imageLinks": "../img/products/minecraft/creeper1.jpg,../img/products/minecraft/creeper1.jpg,../img/products/minecraft/creeper2.jpg,../img/products/minecraft/creeper3.jpg,../img/products/minecraft/creeper4.jpg"
                            },
                            {
                                "productName": "Street Fighter Character Select Men's T-Shirt",
                                "quantity": 100,
                                "productID": "89SF6235",
                                "productCategory": "Apparel",
                                "category": "Vintage Games",
                                "platform": "",
                                "price": 21.99,
                                "description": "Now you can relive the happy times with this retro Street Fighter Character Select men’s cotton tee, featuring the classic character select from the original Street Fighter!",
                                "imageLinks": "../img/products/streetfighter/streetfighter1.png,../img/products/streetfighter/streetfighter1.png"
                            },
                            {
                                "productName": "GTA V GRAND THEFT AUTO FIVE LOGO GRAPHIC T SHIRT",
                                "quantity": 100,
                                "productID": "F5GAD51",
                                "productCategory": "Apparel",
                                "category": "Video Games",
                                "platform": "",
                                "price": 10.0,
                                "description": "GTA V Grand Theft Auto Five Logo T Shirt Custom Printed on the Front Chest.",
                                "imageLinks": "../img/products/gtav/gtav1.jpg,../img/products/gtav/gtav1.jpg,../img/products/gtav/gtav2.jpg,../img/products/gtav/gtav3.jpg,../img/products/gtav/gtav4.jpg"
                            }
                        ]



		ProductResource.getProductsBySearchKeyword(String keyword)
			GET method
			http://localhost:8090/EcommerceAPI/v1/api/products/search/{searchkeyword}
			exp:



                        http://localhost:8090/EcommerceAPI/v1/api/products/search/rpg

                        [
                            {
                                "productName": "Vintage Metroid cotton t-shirt",
                                "quantity": 100,
                                "productID": "56SH682",
                                "productCategory": "Apparel",
                                "category": "RPG",
                                "platform": "",
                                "price": 18.63,
                                "description": "Vintage Metroid cotton t-shirt. The design is based on the action-adventure game Metroid, developed and published by Nintendo.",
                                "imageLinks": "../img/products/metroid/metroid.png,../img/products/metroid/metroid.png,../img/products/metroid/metroid2.png"
                            },
                            {
                                "productName": "Legend of Zelda: Breadth of the Wild",
                                "quantity": 100,
                                "productID": "66LJL89",
                                "productCategory": "Nintendo Switch",
                                "category": "RPG",
                                "platform": "Nintendo Switch",
                                "price": 59.99,
                                "description": "Travel back to Hyrule with Link in Zelda: Breath of the Wild for the Nintendo Switch.\r\nExplore a variety of shrines, sail across rough waters and scavenge for food to defeat Ganondorf and save Zelda once and for all. Zelda: Breath of the Wild 					includes Wolf, who joins Link in his adventure and\r\ndelivers enhanced gameplay for greater enjoyment.",
                                "imageLinks": "../img/products/zelda/zelda.jpg,../img/products/zelda/zelda1.jpg,../img/products/zelda/zeldagameplay.jpg,../img/products/zelda/zeldagameplay2.jpg,../img/products/zelda/zeldagameplay3.png"
                            },
                            {
                                "productName": "Final Fantasy VII Remake",
                                "quantity": 100,
                                "productID": "FFRE4212",
                                "productCategory": "PlayStation 4",
                                "category": "RPG",
                                "platform": "PS4",
                                "price": 59.99,
                                "description": "A spectacular reimagining of one of the most visionary games ever, FINAL FANTASY VII REMAKE rebuilds and expands the legendary RPG for today",
                                "imageLinks": "../img/products/ff7remake/FFVIIRemake.png,../img/products/ff7remake/ff7re01.jpg,../img/products/ff7remake/ff7re02.jpg,../img/products/ff7remake/ff7re03.jpg,../img/products/ff7remake/ff7re04.jpg"
                            }
                        ]


		ShippingResource.getShippingPrice(String shippingmethod)
			GET method
			http://localhost:8090/EcommerceAPI/v1/api/shippingprice/{shippingmethod}
			exp:


                        http://localhost:8090/EcommerceAPI/v1/api/shippingprice/overnight

                        {
                          "shipping_method": "overnight",
                          "price": 11.0
                        }


		StateResource.getTaxRate(String statename)
			GET method
			http://localhost:8090/EcommerceAPI/v1/api/taxrate/{statename}
			exp:

		
                        http://localhost:8090/EcommerceAPI/v1/api/taxrate/California

                        {
                            "tax": 7.25,
                            "state_name": "California"
                        }			
		
		
	//-------------------------------//
		Requirement #3
	//-------------------------------//
			For PA-3, the Gamehub website used Database.java for all of the database calls. For PA-4, all of the Database.java calls have now been replaced by new methods from a new class called "EcommerceAPIService.java" in the "GameHub4" project. The Database.java class now resides in the "EcommerceAPI" project. In "EcommerceAPIService.java" are methods that directly intertact with the "EcommerceAPI" project's REST methods via HTTP request and responses. Both Projects use the same Plain Old Java Object (POJO) classes. Each resource type in the Ecommece API has a corresponding POJO classes to be used as the model class for the resource. The "EcommerceAPIService.java" methods use the JAX-RS objects to build request and most of the methods use the Jackson library for JSON to POJO conversion. In the "EcommerceAPI" project, there are several Resource classes, that use Jeresy JAX-RS REST framework to implement the rest methods. Each Resource class uses a corresponding method in a corresponding Service class. Each Service class has one or many methods that interact with the Database.java class. Each Service class also peforms Map to Pojo conversion to handle the transformation of the different data structures used between database to rest data flow.

 






//-------------------------------//
//	About the site for PA-3
//-------------------------------//

	//Link to the site:
	http://circinus-6.ics.uci.edu:8080/PA-3/

	//MySQL Database 
	Servername = 127.0.0.1
	Username = root
	Passowrd = testdb124
	Databasename = ecommerce

	Requirement #1: All of our Product Category Pages are generated by servlets with the suffix "ProdCat" and all our Product Detail Pages are generated by ProductDetails.java. In the ProductDetails.java, the HttpSession attribute "viewed" is a LinkedList used to store our last 5 products viewed by the user. Each time a user views a product page, this "viewed" attribute add's the id of the product viewed. This session attribute is then later retrieved in the Shopping Cart page generated by the servlet ShoppingCart.java.

	Requirement #2: As mentioned above in requirement #1 description, all product detail pages are generated by a single servlet ProductDetails.java. Each product details page is provided a product id and is assembled with the related product information retrieved from the ecommerce database's product table given the provided product id. In each product details page is an "Add to Cart" button, which when clicked, stores the item information such as product id and quantity into a HttpSession attribute "cart". This is a Map<String, Integer> that stores all of the products and their corresponding quantities as a collection. The shopping cart page is generated by the servlet ShoppingCart.java and retrieves the http session attribute "cart" to build out a table of the shopping cart items. The shopping cart also include a calculate order summary based on the items in the shopping cart.

	Requirement #3: After a user visit the shopping cart, they can click the "PROCEED TO CHECKOUT" button, this will take them to the checkout page. The checkout page is generated by the servlet Checkout.java. In here the shopping cart table and order summary is recreated using data from the http session attribute "cart". to the right of the order summary and shopping cart is the orderform. In here the user enters information such as shipping and billing and when they are ready they can click the "SUBMIT" button. When clicking the "SUBMIT" button, the Checkout.java servlet sends a post requst to the Order Processing servlet OrderProcessing.java. A post method is used to secure customer sensitive information such as credit card and address. In the order processing servlet, sub totals, shipping, taxes and final total is calculated. The  order informatin is then stored into the ecommerce databases' order table. The database.java class handles all databases calls, whether its data retrieveal or insertion. All sql statements use prepared statements to prevent sql injection. Upon completion of order processing and order record insertion, the order processing servlet, uses RequestDispatcher's forward method, to forward the http request to the Order Confirmation servlet OrderConfirmation.java. In the OrderConfirmation.java servlet, the order confirmation is generated and displayed the customer.  The page includes the date the order was made, a table of all the products purchased and their quanitites. It includes subtotals, discount, shipping, tax and final total information. It also provides a billing and shipping summary fo the customer.



//-------------------------------//
//	About the site for PA-2
//-------------------------------//

	//Link to the site:
	http://circinus-6.ics.uci.edu:8081/PA-2/

	//MySQL Database 
	Servername = 127.0.0.1
	Username = root
	Passowrd = testdb
	Databasename = ecommerce


	Our website is a conversion of our previous website (See "About the site for PA-1" description below) from static pages to dynamically generated pages.
	The website is now hosted on Apache web server, to support PHP.
	To enable dynamic generation, we now use a MySQL database to store product information.
	The database is also used to store customer order information as well.
	Every webpage has now been converted from html to php (except the index.html, which is a static page).
	Since, all of the pages comprise of the same header and a footer, the header and footer are now dynamically created by thse same 
        header.php and footer.php files, eliminating the redundancy in PA-1. This way, if a change needs to be done on either the header 
	and footer, the code change only needs to be performed on a single file, and all the changes will cascade to all files using those php files.
	The product page, displays several product categories, each represented by a category card.
	When clicked, the user is directed to that product category's page. In here, the related products are displayed in the form of product cards.
	Since each product category page can contain a variable number of products, this page now dynamically generates the product cards,
	using the product records stored in the product table.
	All php files that use database calls, import the file database.php
	In database.php, all of the user defined database functions are centrally located.
	We use the PDO (PHP Data Objects) approach, as opposed to the MySQLi extension approach, in implementing the database calls.
	Every single sql statment, when executed, begins with a openning connection and closing connection, to ensure minimization of the number of uncessary connection threads.
	When the user selects a product card, displayed in the product category page, the user is directed to the product's details page.	
	All product details pages are dyamically generated from a single file named "productdetails.php"
	This page, takes a single product id, passed from the previous page via get method, and then queries the ecommerce database, retrieve the related product data, and is used to build the product details page according to the product id.
	Every product details page is accompanied with a product order form.
	When a customer fills out the order form and submits the form, upon submisson, first the onsumbit event handler calls the validate() function to perform client-side validation, which is located in productdetail.js, in here the javascript checks for empty fields, and peforms regular expressions matching to validate email format. In the previous version of the website (PA-1) after javascript validation, an email was created containing the product and order information. This is now been disabled in favor of a order confirmation page.
	After the validate() funcition is finished, the form is configured to send a post method to the ordervalidation.php file.
	The page is redirected to the ordervalidation.php and server-side validaton is performed on the individual form fields.
	The validate_input() funcition is called on all form fields, and validation involves the stripping of unnecessary characters, removal of backslashes, and escaping HTML code to prevent Cross-site Scripting (XSS).
	After validation, the order's subtotal, shipping, taxes and final total are calculated.
	The order information is then stored into the order table of the database, using the setOrder() function.
	The setOrder() database function, uses a prepared statement to prevent sql injection.
	For ajax, our website uses ajax in two different areas. We use ajax in our product search page and we use ajax in our product order form.
	For the product search page, when a user enters a keyword, in the search box, keykup event listener is triggered and a post method is sent to the getProducts.php file.
	Each time a key is pressed, the search page's content displays related products in table format. The products in the table provide a link to the their respective product detail pages.
	For the second ajax feature, when a user is data entering the order form, ajax is used to assist the user an entering information.
	When the user enters a character in the state field, auto complete capability is provided to suggest possible states to the user. 
	When the user enters a number in the zip code field, auto complete capability is provided to suggest possible zipcodes to the user, when a user selects a zip code, the city and state pertaining to that zip code, are auto populated in their respective fields.
	To implement this, the event listeners point to either getState.php or getZip.php. Inside of these files are database calls to the database.php file.
	We also implemented the state and zipcode table in the ecommerce database, to store the information needed for auto suggestion in the form fields. 
		


//-------------------------------//	
//	About the site for PA-1
//-------------------------------//
	Our website, is an ecommerce site, that is an online retailer for all things video game related.
	We have a cool introducing home page where the user must click the floating bar to enter the actual e-commerce page.
	The products page shows the overview about our products, along with the general categories of merchandise that 
	we are selling.
	The top navigation bar also provide a about drop down button, where our team, company and contact pages are located.
	The company page gives the overview about the site, and team page gives relative information about the team
	and also the student ids. The contact page also shows a email option that could send contact request.
	From the product page the user can choose any category he or she is interested and upon clicking on the category picture,
	the customer enters the product category page, which display the related products. 
	The category and product cards have hover effect which zooms out when a mouse is being hovered on.
	The customer can click on the individual product's picture, to enter the product's details page.
	The detailed page shows all basic information about the product, the user can click on the small pictures to 
	enlarge them.
	The ordering form can also be found on each of the product detail pages, upon submission, it checks each field and peforms 	
	validation. After validation an email is sent confirming the purchase order.
	
//References for PA-1

	
	//How to Top Nav
	https://www.w3schools.com/howto/howto_js_topnav.asp

	//The Joy of CSS Grid - Build 3 Beautifully Simple Responsive Layouts
	https://www.youtube.com/watch?v=705XCEruZFs

	//Search Bar
	https://www.w3schools.com/howto/howto_css_fullscreen_search.asp

	//A step-by-step guide to getting started with HTML tables
	https://medium.com/free-code-camp/a-step-by-step-guide-to-getting-started-with-html-tables-7f43b18f962b

	//How To Create a Card
	https://www.w3schools.com/howto/howto_css_cards.asp

	//How TO - Parallax Scrolling
	https://www.w3schools.com/howto/howto_css_parallax.asp

	//HOW TO - Blurred Background Image
	https://www.w3schools.com/howto/howto_css_blurred_background.asp

	//HOW TO - Meet the team
	https://www.w3schools.com/howto/howto_css_team.asp
	
	//HOW TO - CSS/JS Modal
	https://www.w3schools.com/howto/howto_css_modals.asp
	
	//game controller icon
	https://fontawesome.com/icons/gamepad?style=solid

	//marquee //end up removing date and time
	http://jsfiddle.net/n2XB6/63/

	//JavaScript - Form Validation
	https://www.tutorialspoint.com/javascript/javascript_form_validations.htm

	//JavaScript: HTML Form - email validation
	https://www.w3resource.com/javascript/form/email-validation.php

	//Automatically open default email client and pre-populate content
	https://stackoverflow.com/questions/13231125/automatically-open-default-email-client-and-pre-populate-content


//References for PA-2
	//Making Dynamic Website With PHP
	https://www.youtube.com/watch?v=pzwqgVSGAuU

	//How to generate a HTML page dynamically using PHP?
	https://stackoverflow.com/questions/14871014/how-to-generate-a-html-page-dynamically-using-php
	

	//MySQL Tutorial for Beginners | How to create database and add table in XAMPP using MYSQL
	https://www.youtube.com/watch?v=WmFHreVid-k

	//Importing MySQL databases and tables using phpMyAdmin
	https://www.youtube.com/watch?v=jW5lrS6EUPM

	//Exporting MySQL databases and tables using phpMyAdmin
	https://www.youtube.com/watch?v=ffmCGVse8yg

	//How to rename database tables in phpMyAdmin
	https://www.youtube.com/watch?time_continue=3&v=ompnBfAP43A&feature=emb_logo
		select table to rename
		click on the operations tab, and scroll down to the "table options" section		
		rename the table in the "Rename table to" field
		click go
		you should see the table name changes.

	//36: How to connect to a database in PHP | PHP tutorial | Learn PHP programming
	https://www.youtube.com/watch?v=ILyf16MEvHM

	//37: How to show database data on a website using MySQLi | PHP tutorial | Learn PHP programming
	https://www.youtube.com/watch?v=0YLJ0uO6n8I


	//PHP Functions
	https://www.tutorialrepublic.com/php-tutorial/php-functions.php

	//PHP Functions returning value
	https://www.tutorialspoint.com/php/php_functions.htm

	//PHP Connect to MySQL
	https://www.w3schools.com/php/php_mysql_connect.asp

	//How to call function of one php file from another php file and pass parameters to it?
	https://stackoverflow.com/questions/8104998/how-to-call-function-of-one-php-file-from-another-php-file-and-pass-parameters-t

	//How to Connect MySQL Database with PHP Websites
	https://www.cloudways.com/blog/connect-mysql-with-php/#connection

	//How do I loop through a MySQL query via PDO in PHP?
	https://stackoverflow.com/questions/159924/how-do-i-loop-through-a-mysql-query-via-pdo-in-php


------>	//Creating a dynamic one-page website using PHP and mySQL
	https://www.youtube.com/playlist?list=PL_QXkBHy8yVo085n2_3T9S0h8vfW-ksVp

	04292020: Had to update product table to include a new column called "ProductCategory"
	this is associate the product by which category page they will belong to.

	//Loop Through an Associative Array
	https://www.w3schools.com/php/php_arrays_associative.asp

	//Foreach loop through multidimensional array in PHP
	https://www.tutorialrepublic.com/faq/foreach-loop-through-multidimensional-array-in-php.php

	//3 ways to display divs side by side (float, flexbox, CSS grid)
	https://coder-coder.com/display-divs-side-by-side/
	

	//HTML <input> readonly Attribute
	https://www.w3schools.com/tags/att_input_readonly.asp

	//16. Adding a record to a MySQL database using a web form
	https://www.youtube.com/watch?v=4-bs2DlBoKs&list=PL_QXkBHy8yVo085n2_3T9S0h8vfW-ksVp&index=17&t=0s

	//17. Adding confirmation functionality to our web form		//didn't implement this, it uses $_SESSION
	https://www.youtube.com/watch?v=GZ9EJ0Iky6M&list=PL_QXkBHy8yVo085n2_3T9S0h8vfW-ksVp&index=17

	//A form's “action” and “onsubmit”: Which executes first?
	https://stackoverflow.com/questions/29014570/a-forms-action-and-onsubmit-which-executes-first

	//Retrieve post array values
	https://stackoverflow.com/questions/21750478/retrieve-post-array-values

	//PHP MySQL Insert Data
	https://www.w3schools.com/php/php_mysql_insert.asp

	//INSERT query using PDO
	https://phpdelusions.net/pdo_examples/insert

	//Need to add more fields to the order table
		orderID //primary key, int, auto-increment

		//How To add auto increment column in MySQL (phpMyAdmin) existing table
		https://www.youtube.com/watch?v=BEaP4tUagK4

		//Best Data Type to store Money in MySQL?
		https://rietta.com/blog/best-data-types-for-currencymoney-in/
	
	//The PHP switch Statement
	https://www.w3schools.com/php/php_switch.asp

	//PHP Operators
	https://www.w3schools.com/php/php_operators.asp

	//Get DATETIME in php and post it to MySQL for transaction consistency
	https://stackoverflow.com/questions/10129180/get-datetime-in-php-and-post-it-to-mysql-for-transaction-consistency

	//Insert current date in datetime format mySQL
	https://stackoverflow.com/questions/9541029/insert-current-date-in-datetime-format-mysql

	//PHP Form Validation
	https://www.w3schools.com/php/php_form_validation.asp

	//When to use single quotes, double quotes, and backticks in MySQL
	https://stackoverflow.com/questions/11321491/when-to-use-single-quotes-double-quotes-and-backticks-in-mysql

	//"Reload-1s-200px.gif" was generated and downloaded from "loading.io"
	https://loading.io/spinner/reload/-reload-sync-ajax-get-fetch

	//Open another page in php [duplicate]
	https://stackoverflow.com/questions/7865984/open-another-page-in-php

	//Page redirect after certain time PHP
	https://stackoverflow.com/questions/6119451/page-redirect-after-certain-time-php

	//CSS: Position loading indicator in the center of the screen
	https://stackoverflow.com/questions/6256043/css-position-loading-indicator-in-the-center-of-the-screen

	//PHP MySQL Get Last Inserted ID
	https://www.w3schools.com/php/php_mysql_insert_lastid.asp

	//PDO get the last ID inserted
	https://stackoverflow.com/questions/10680943/pdo-get-the-last-id-inserted

	//jQuery Tutorial for Beginners		//Playlist 10 videos
	https://www.youtube.com/playlist?list=PLoYCgNOIyGABdI2V8I_SWo22tFpgh2s6_

	//SELECT query with PDO
	https://phpdelusions.net/pdo_examples/select

	//Import CSV to existing MySQL table using PhpMyAdmin
	https://www.youtube.com/watch?v=xppyP5ibbYM


	//AJAX Introduction
	https://www.w3schools.com/xml/ajax_intro.asp

	//Ajax dynamic text box field update
	https://forum.jquery.com/topic/ajax-dynamic-text-box-field-update

	//Ajax Live Data Search using Jquery PHP MySql
	https://www.youtube.com/watch?v=wCsPAquMNVw

	//Ajax Live Data Search using Jquery PHP MySql
	https://www.webslesson.info/2016/03/ajax-live-data-search-using-jquery-php-mysql.html

	//How to create an HTML button that acts like a link?
	https://stackoverflow.com/questions/2906582/how-to-create-an-html-button-that-acts-like-a-link

	//How do I create a PDO parameterized query with a LIKE statement?
	https://stackoverflow.com/questions/583336/how-do-i-create-a-pdo-parameterized-query-with-a-like-statement

	//How to count all elements or values in an array in PHP
	https://www.tutorialrepublic.com/faq/how-to-count-all-elements-in-an-array-in-php.php

	//How do I print all POST results when a form is submitted? [duplicate]
	https://stackoverflow.com/questions/9332718/how-do-i-print-all-post-results-when-a-form-is-submitted
	
	//How do I import an SQL file using the command line in MySQL?
        https://stackoverflow.com/questions/17666249/how-do-i-import-an-sql-file-using-the-command-line-in-mysql

	//How to "Import SQL file to MySQL"
	https://www.youtube.com/watch?v=T3A-wfqtjZA

	//How to Use the Linux Screen Tool
	https://www.youtube.com/watch?v=3txYaF_IVZQ

//References for PA-3
	//SHOPPING CART WITH SESSION IN JSP-SERVLET
	http://learningprogramming.net/java/jsp-servlet/shopping-cart-with-session-in-jsp-servlet/

	//Introduction to Java Servlets
        https://www.youtube.com/watch?v=uQzedmaebvk


	//Servlet and JSP Tutorial for Beginners 2018 by Telusko
	https://www.youtube.com/watch?v=7TOmdDJc14s&list=PLsyeobzWxl7pUPF2xjjJiG4BKC9x_GY46

	//How to Install Apache Tomcat on Mac OS X
	https://www.youtube.com/watch?v=h_qQOVDTxo8
	Go to https://tomcat.apache.org/download-90.cgi   //Install Tomcat-9

	//How to download and install MySQL for MAC OS X | 2019
	https://www.youtube.com/watch?v=9sbUsbDWTE8

	//How to import SQL file to MySQL by using MySQL Workbench - MySQL DBA Tutorial
	https://www.youtube.com/watch?v=o-eO_SnuZXU


	//Deploying just HTML, CSS webpage to Tomcat
	https://stackoverflow.com/questions/3954621/deploying-just-html-css-webpage-to-tomcat



	//What is JDBC?
	https://www.tutorialspoint.com/jdbc/jdbc-quick-guide.htm

	/What is the MySQL JDBC driver connection string?
	https://stackoverflow.com/questions/1457716/what-is-the-mysql-jdbc-driver-connection-string

	//Downloading Mysql Connector Jar File ||Mysql Jar File
	https://www.youtube.com/watch?v=hIzSiGpAXH0


	/How to return an html document from java servlet? [duplicate]
	https://stackoverflow.com/questions/17036130/how-to-return-an-html-document-from-java-servlet/17036152


	//Printing the result of ResultSet
	https://coderwall.com/p/609ppa/printing-the-result-of-resultset

	//Efficient way to Handle ResultSet in Java
	https://stackoverflow.com/questions/7507121/efficient-way-to-handle-resultset-in-java

	//Creating an ArrayList of Maps
	https://codereview.stackexchange.com/questions/112201/creating-an-arraylist-of-maps

	//Hashmap holding different data types as values for instance Integer, String and Object
	https://stackoverflow.com/questions/14599317/hashmap-holding-different-data-types-as-values-for-instance-integer-string-and

	//Java iterate over ArrayList with HashMap in it
	https://stackoverflow.com/questions/27485794/java-iterate-over-arraylist-with-hashmap-in-it

	//Iterate through a HashMap [duplicate]
	https://stackoverflow.com/questions/1066589/iterate-through-a-hashmap


	//How to generate the whole database script in MySQL Workbench - MySQL DBA Tutorial
	https://www.youtube.com/watch?v=N_x0a7LgU5M


	//How to change a value of a field using MySQL Workbench?
	https://stackoverflow.com/questions/33176332/how-to-change-a-value-of-a-field-using-mysql-workbench
	
	//"SHOPPING CART WITH SESSION IN JSP-SERVLET"
	http://learningprogramming.net/java/jsp-servlet/shopping-cart-with-session-in-jsp-servlet/

	//
	https://cs.gmu.edu/~offutt/classes/642/examples/servlets/
	https://cs.gmu.edu/~offutt/classes/642/examples/servlets/catalog.java
	https://cs.gmu.edu/~offutt/classes/642/examples/servlets/shoppingCart.java

	//Reading Form Data using JSP
	https://www.tutorialspoint.com/jsp/jsp_form_processing.htm


	//How to transfer data from JSP to servlet when submitting HTML form
	https://stackoverflow.com/questions/4971877/how-to-transfer-data-from-jsp-to-servlet-when-submitting-html-form

	//Accessing post variables using Java Servlets
	https://stackoverflow.com/questions/5222/accessing-post-variables-using-java-servlets

	//How to view all columns in a database in MySQL Workbench
	https://dataedo.com/kb/tools/mysql-workbench/view-all-columns-in-a-database

	//How to retrieve Date from a table in JDBC?
	https://www.tutorialspoint.com/how-to-retrieve-date-from-a-table-in-jdbc


	//HOW TO CONVERT JAVA.UTIL.DATE TO MYSQL DATE FORMAT
	https://theopentutorials.com/examples/java/util/date/how-to-convert-java-util-date-to-mysql-date-format/

	//How to get the insert ID in JDBC?
	https://stackoverflow.com/questions/1915166/how-to-get-the-insert-id-in-jdbc

	//SQLException - Generated keys not requested (MySQL)
	https://stackoverflow.com/questions/7162989/sqlexception-generated-keys-not-requested-mysql

	//JDBC - Insert Records Example
	https://www.tutorialspoint.com/jdbc/jdbc-insert-records.htm

	//JDBC PreparedStatement – Insert a row
	https://mkyong.com/jdbc/jdbc-preparestatement-example-insert-a-record/

	//Adding new column via MySQL Workbench into existing table
	https://stackoverflow.com/questions/46724428/adding-new-column-via-mysql-workbench-into-existing-table

	//Java Hashmap: How to get key from value?
	https://stackoverflow.com/questions/1383797/java-hashmap-how-to-get-key-from-value

	//Is there a Util to convert US State Name to State Code. eg. Arizona to AZ?
	https://stackoverflow.com/questions/11005751/is-there-a-util-to-convert-us-state-name-to-state-code-eg-arizona-to-az



//Referencesfor PA-4
	

	//JAX-RS Example Jersey
	https://www.javatpoint.com/jax-rs-example-jersey

	//READ Build RESTful Service in Java using JAX-RS and Jersey (Celsius to Fahrenheit & Fahrenheit to Celsius)
	https://crunchify.com/how-to-build-restful-service-with-java-using-jax-rs-and-jersey/

	//Another Todo: http://www.vogella.com/tutorials/REST/article.html

	//Priniples of a good RESTful API design (mandatory reading):
	https://codeplanetio.wordpress.com/2013/12/31/principles-good-restful-api-design/

	//Simple example for using JSON: 
	http://www.mykyong.com/webservices/jax-rs/json-example-with-jersey-jackson

	//Tutorials
	Servlet and JSP Tutorial | Why JSP?
	https://www.youtube.com/watch?v=5Sz8z1Yreow&list=PLsyeobzWxl7pUPF2xjjJiG4BKC9x_GY46&index=16



	//JSP equivalent of ASP.NET MVC's partial views?
	https://www.youtube.com/watch?v=R7bkBD_EXOI


	//How to forward a request from one Jsp to another Jsp ?.
	https://www.hubberspot.com/2012/10/how-to-forward-request-from-one-jsp-to.html

	//Java String replace()
	https://www.javatpoint.com/java-string-replace



	//REST with Java (JAX-RS) using Jersey - Tutorial
	https://www.vogella.com/tutorials/REST/article.html


	

	//Hello World Rest Webservice With Jersey API
	https://www.youtube.com/watch?v=AWrKBJ8mHz0&t=338s


	//Using “like” wildcard in prepared statement
	https://stackoverflow.com/questions/8247970/using-like-wildcard-in-prepared-statement

	//MySQL LIKE
	https://www.mysqltutorial.org/mysql-like/



	//Rest API | Web Service Tutorial
	https://www.youtube.com/watch?v=BZi44GOD8kY


	//Hello World Rest Webservice With Jersey API
	https://www.youtube.com/watch?v=AWrKBJ8mHz0&t=338s

	//REST with Java (JAX-RS) using Jersey - Tutorial
	https://www.vogella.com/tutorials/REST/article.html

	//Build RESTful Service in Java using JAX-RS and Jersey (Celsius to Fahrenheit & Fahrenheit to Celsius)
	https://crunchify.com/how-to-build-restful-service-with-java-using-jax-rs-and-jersey/


	//What is a POJO Class?
	https://www.baeldung.com/java-pojo-class


	//Intro to the Jackson ObjectMapper
	https://www.baeldung.com/jackson-object-mapper-tutorial

	//Consuming REST services from Java applications
	https://vaadin.com/learn/tutorials/consuming-rest-services-from-java-applications

	//Intro to the Jackson ObjectMapper
	https://www.baeldung.com/jackson-object-mapper-tutorial


	//POST Request using Postman
	https://www.toolsqa.com/postman/post-request-in-postman/


	//return newly generated id restful service post
	https://stackoverflow.com/questions/12971236/return-newly-generated-id-restful-service-post

	//Read response body in JAX-RS client from a post request
	https://stackoverflow.com/questions/18086621/read-response-body-in-jax-rs-client-from-a-post-request







